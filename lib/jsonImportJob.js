...
async function runJsonImport({ jsonText, targetChannel, statusChannel, delayMs = 1200, maxItems = 200 }) {
    ...
    const toSend = itemKeys.filter((k) => !existingSet.has(k)).map((k) => ({ key: k, item: keyToItem.get(k) }));

    if (statusChannel?.isTextBased?.()) {
        await statusChannel.send(
            `📥 Import JSON: ${items.length} trouvé(s) · ${toSend.length} nouveau(x) vers ${targetChannel} (délai ${delayMs}ms).`
        );
    }
