...
async function handleVipProofMessage(message) {
    ...
    if (n < min || n > max) {
        const failText = `❌ Il faut envoyer **entre ${min} et ${max}** pièces jointes dans **un seul** message.\nTu en as envoyé **${n}**.`;
        await message.reply({ content: failText, allowedMentions: { repliedUser: true } }).catch(() => {});
        return;
    }
    ...
    const titleRaw = (cfg.vipProofEmbedTitle || '').trim();
    const title = titleRaw ? applyVipTemplate(titleRaw, ctx).slice(0, 200) : '';
    const body = applyVipTemplate(
        cfg.vipProofEmbedDescription ||
            'Bravo {mention} ! Ta preuve est acceptée. Rôle {role} jusqu’au <t:{expires}:F> ({nb_fichiers} fichier(s)).',
        ctx
    );
    let content = title ? `**${title}**\n\n${body}` : body;
    content = content.slice(0, 2000);

    await message.reply({ content, allowedMentions: { users: [message.author.id] } }).catch(() => {});
}
