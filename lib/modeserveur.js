...
/**
 * DEVERROUILLAGE FORCÉ : enlève les surcharges @everyone + rôles cibles sur chaque salon
 * (héritage catégorie / serveur — comme avant des réglages par salon pour ces sujets).
 */
async function forceUnlockAllChannels(guild, targetRoleIds) {
    const everyoneId = guild.id;
    const roleIds = [...new Set((targetRoleIds || []).filter((id) => id && id !== everyoneId))];
    ...
    for (const ch of sortedVitrineChannels(guild)) {
        ...
        for (const rid of roleIds) {
            if (!guild.roles.cache.has(rid)) continue;
            try {
                await ch.permissionOverwrites.delete(rid);
                n += 1;
            } catch {
                if (await clearViewChannelToInherit(ch, rid)) n += 1;
            }
